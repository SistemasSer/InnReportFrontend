...
function PucSup() {
  const { pucSup } = useSelector((state: { pucSup: PucSupState }) => state);

  const pucSupService = new PucSupService();
  ...
  const fetchData = async () => {
    try {
      const res: IPucSup[] = await pucSupService.getAll();
      dispatch(setPucSup(res));
    } catch (error) {
  ...
  return (
    <div className="block mt-0 xl:w-screen px-3 py-3 bg-transparent">
